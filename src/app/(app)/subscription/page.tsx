...
function PageSkeleton() {
  return (
    <div className="px-4 sm:px-8 pt-4 sm:pt-6 pb-8 sm:pb-10 space-y-5 sm:space-y-6 animate-in fade-in duration-300">
      <Sk className="h-5 sm:h-6 w-40 sm:w-52" />
      <Sk className="h-[250px] sm:h-36 w-full rounded-lg" />
      <Sk className="h-16 sm:h-20 w-full rounded-lg" />
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">{[1,2,3,4].map(i => <Sk key={i} className="h-48 sm:h-56 rounded-lg" />)}</div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      {dailyUsed !== undefined && dailyLimit !== undefined && (
        <p className="text-[9px] sm:text-[10px] text-slate-600">{dailyUsed}/{dailyLimit} today</p>
      )}
    </div>
  );
}
...
function PlanCard({ plan, isCurrentPlan, isPendingPlan, isBusy, onSubscribe }: {
  ...
  return (
    <div className={cn(
      ...
      <div className="p-3.5 sm:p-4 flex flex-col flex-1 gap-3 sm:gap-3.5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2 sm:gap-2.5">
            <div className={cn("h-6 w-6 sm:h-7 sm:w-7 rounded-md flex items-center justify-center border", isCurrentPlan ? "bg-violet-500/15 border-violet-500/25" : "bg-violet-500/10 border-violet-500/10")}>
              <Icon className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-violet-400" />
            </div>
            <div>
              <p className="text-xs sm:text-sm font-semibold text-white leading-none">{plan.name.replace("Pentellia ", "")}</p>
              {isPopular && !isCurrentPlan && (
                <span className="text-[9px] sm:text-[10px] font-medium text-violet-400 flex items-center gap-0.5 mt-0.5">
                  <Star className="h-2 w-2 sm:h-2.5 sm:w-2.5 fill-violet-400" /> Popular
                </span>
              )}
            </div>
          </div>
          {isCurrentPlan && <span className="text-[9px] sm:text-[10px] font-semibold text-violet-300 bg-violet-500/15 border border-violet-500/25 px-1.5 py-0.5 rounded">Active</span>}
          {isPendingPlan && <span className="text-[9px] sm:text-[10px] font-semibold text-amber-300 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded">Scheduled</span>}
        </div>

        <div className="flex items-baseline gap-1">
          <span className="text-[10px] sm:text-xs text-slate-500">₹</span>
          <span className="text-lg sm:text-xl font-bold text-white">{plan.price_inr.toLocaleString("en-IN")}</span>
        ...
        <ul className="space-y-1.5 flex-1">
          {[
            ...
            <li key={i} className="flex items-center gap-1.5 sm:gap-2">
              <Check className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-violet-500/60 shrink-0" />
              <span className="text-[11px] sm:text-xs text-slate-400">{f}</span>
            </li>
          ))}
        </ul>
        ...
        <button
          ...
          className={cn(
            "w-full h-7 sm:h-8 rounded-md text-[11px] sm:text-xs font-medium transition-all flex items-center justify-center gap-1.5",
        ...
        >
          {isBusy ? <><Loader2 className="h-3 w-3 sm:h-3.5 sm:w-3.5 animate-spin" /> Processing</>
          : isCurrentPlan ? <><Check className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Current Plan</>
          : <>Select <ChevronRight className="h-2.5 w-2.5 sm:h-3 sm:w-3" /></>}
        </button>
      </div>
    </div>
  );
...
export default function SubscriptionPage() {
  ...
  const handleDownloadInvoice = handleViewReceipt; 
  ...
  return (
    <div className="px-4 sm:px-8 pt-4 sm:pt-6 pb-8 sm:pb-10 space-y-5 sm:space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        <h1 className="text-lg sm:text-xl font-semibold text-white tracking-tight">Billing & Subscription</h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-0.5 sm:mt-1">Manage your plan, usage, and payment history.</p>
      </div>
      ...
      {hasActivePlan && currentSub ? (
          ...
          <div className="p-4 sm:p-5">
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              <div className="flex items-start gap-3 sm:gap-4 flex-1">
                <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-lg bg-violet-500/10 border border-violet-500/15 flex items-center justify-center shrink-0">
                  {React.createElement(PLAN_ICONS[currentSub.planId] ?? Shield, { className: "h-4 w-4 sm:h-5 sm:w-5 text-violet-400" })}
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm sm:text-base font-semibold text-white">{currentSub.planName}</p>
                    <span className={cn("text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded border",
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 mt-1 sm:mt-1.5 flex-wrap">
                    ...
                    <span className={cn("text-[11px] sm:text-xs font-medium", isExpiring ? "text-amber-400" : "text-slate-400")}>
                      {daysLeft} day{daysLeft !== 1 ? "s" : ""} remaining
                    </span>
                  </div>
                  <div className="mt-2 sm:mt-3 h-1.5 w-full sm:max-w-xs rounded-full bg-white/[0.06] overflow-hidden">
              ...
              <button
                ...
                className="flex items-center justify-center sm:justify-start gap-2 px-4 h-9 rounded-md border border-white/[0.08] bg-white/[0.04] hover:bg-white/[0.07] text-[13px] sm:text-sm text-slate-300 transition-all w-full sm:w-auto shrink-0 mt-2 sm:mt-0"
              >
            ...
            {pendingPlan && (
              <div className="mt-4 flex items-start gap-2 sm:gap-2.5 p-2.5 sm:p-3 rounded-md bg-amber-500/[0.06] border border-amber-500/20">
                <Info className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-400 shrink-0 mt-0.5" />
            ...
            {usage && (
              <div className="mt-4 pt-4 border-t border-white/[0.05]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[11px] sm:text-xs font-medium text-slate-400">Current Period Usage</p>
                  <span className="ml-auto text-[9px] sm:text-[10px] text-slate-600">resets on plan renewal</span>
                </div>
        ...
        <div className="rounded-lg border border-violet-500/20 bg-violet-500/[0.05] p-4 sm:p-5 flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
          <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-md bg-violet-500/15 border border-violet-500/20 flex items-center justify-center shrink-0">
            <Zap className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-violet-400" />
          </div>
          <div>
            <p className="text-[13px] sm:text-sm font-semibold text-white">No active subscription</p>
      ...
      {(!hasActivePlan || showPlans) && (
        <div className="animate-in slide-in-from-top-1 fade-in duration-200 space-y-3">
          ...
          <p className="text-[10px] sm:text-[11px] text-slate-600 flex items-center gap-1.5">
            <Lock className="h-2.5 w-2.5 sm:h-3 sm:w-3" /> Payments by Razorpay · 30-day cycles · No auto-renewal
          </p>
        </div>
      )}
      ...
      {payments.length > 0 ? (
        <div className="rounded-lg border border-white/[0.07] bg-[#0d0e1a] overflow-hidden">
          <div className="px-4 sm:px-5 py-3 sm:py-3.5 border-b border-white/[0.05] flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Receipt className="h-3.5 w-3.5 text-slate-500" />
              <p className="text-[11px] sm:text-xs font-semibold text-slate-300 uppercase tracking-wider">Payment History</p>
            </div>
            <span className="text-[10px] sm:text-[11px] text-slate-600">{payments.length} record{payments.length !== 1 ? "s" : ""}</span>
          </div>
          ...
          <div className="divide-y divide-white/[0.04]">
            {payments.map(p => {
              ...
              return (
                <div key={p.razorpay_order_id} className="px-4 sm:px-5 py-3 sm:py-3.5 hover:bg-white/[0.02] transition-colors group">
                  {/* Mobile */}
                  <div className="sm:hidden flex items-center justify-between">
                    <div className="flex-1 pr-2">
                      <p className="text-[13px] text-slate-200 font-medium truncate">{planLabel} Plan</p>
                      <p className="text-[10px] text-slate-500 font-mono mt-0.5 truncate">{p.razorpay_payment_id}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      <p className="text-xs font-semibold text-slate-200">₹{Number(p.amount_inr).toLocaleString("en-IN")}</p>
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleViewReceipt(p.razorpay_payment_id)} disabled={isDown}
                          className="h-6 w-6 flex items-center justify-center rounded-md text-slate-500 hover:text-violet-400 hover:bg-violet-500/10 transition-all bg-white/[0.03]">
                          {isDown ? <Loader2 className="h-3 w-3 animate-spin" /> : <FileText className="h-3 w-3" />}
                        </button>
        ...
        <div className="rounded-lg border border-white/[0.07] bg-[#0d0e1a] p-6 sm:p-8 flex flex-col items-center gap-3 text-center">
          <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-lg bg-white/[0.04] flex items-center justify-center">
            <Receipt className="h-4 w-4 sm:h-5 sm:w-5 text-slate-600" />
          </div>
          <p className="text-[13px] sm:text-sm text-slate-400 font-medium">No payments yet</p>
          <p className="text-[11px] sm:text-xs text-slate-600">Your payment history will appear here after subscribing.</p>
        </div>
      )}
      ...
      {receiptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm"
          ...
          <div className="relative w-full max-w-[95%] sm:max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-xl overflow-hidden shadow-2xl border border-white/10"
            onClick={e => e.stopPropagation()}>
            {/* Modal header */}
            <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 bg-[#0d0e1a] border-b border-white/[0.07]">
              <div className="flex items-center gap-2 sm:gap-2.5 truncate pr-2">
                <Receipt className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-violet-400 shrink-0" />
                <span className="text-[13px] sm:text-sm font-semibold text-white truncate">Invoice {receiptModal.invoiceNumber}</span>
              </div>
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                <button
                  onClick={() => handleDownloadPdf(receiptModal.paymentId)}
                  className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 text-[10px] sm:text-xs font-medium text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg border border-white/10 transition-all">
                  <Download className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> <span className="hidden sm:inline">Download</span> PDF
                </button>
                <button onClick={() => setReceiptModal(null)}
                  className="h-7 w-7 sm:h-8 sm:w-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-white hover:bg-white/10 transition-all">
