...
// Phase 3: DomainVerificationProvider REMOVED. UnverifiedBanner REMOVED.
// DomainVerificationModal REMOVED. Added SubscriptionBanner for expired plans.
...
function SubscriptionBanner() {
  ...
  if (!data.subscription) {
    return (
      <div className="mx-4 mt-4 mb-0 rounded-xl border border-violet-500/25 bg-violet-500/[0.07] px-4 py-3 flex items-center gap-3">
        ...
        <Link
          href="/subscription"
          className="flex items-center gap-1 text-xs font-semibold text-violet-300 hover:text-white transition-colors shrink-0"
        >
  ...
  return (
    <div className={cn(
      "mx-4 mt-4 mb-0 rounded-xl border px-4 py-3 flex items-center gap-3",
      isExpired
      ...
      <Link
        ...
        className={cn(
          ...
          isExpired ? "text-red-300 hover:text-white" : "text-amber-300 hover:text-white",
        )}
      >
    ...
    <div className="relative h-screen w-full bg-[#05050A] font-sans text-slate-200 selection:bg-violet-500/30 overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex h-full pt-16">
        ...
        <main className={cn(
          ...
          <SubscriptionBanner />
          <div className={`${path.includes("scans") ? " " : "p-8"} animate-in fade-in slide-in-from-bottom-4 duration-500 min-h-full`}>
            {children}
          </div>
        </main>
      </div>
